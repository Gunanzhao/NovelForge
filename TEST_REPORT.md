...
## 1.1.1-rc.1 发布验证（2026-09-08）

本版由本地 rc.14 改号，业务代码保持一致。发布内容、验证边界与下载见 [发布说明](docs/releases/v1.1.1-rc.1.md)，云端门禁结果见对应源码提交的 GitHub Actions。以下 rc.14–rc.11 条目为历史本地验收记录。


## rc.14 本地验收（2026-09-08）

该次本地验收版本为 1.1.0-rc.14，以下为历史结果。

- TypeScript、ESLint、前端 56 文件 / 330 项全部通过。
- 实际 rc.14 桌面在检查过程中切换页面、连接成功后切换正文／总览／项目设置，以及切换离线模式后返回 Codex，均保留检查与连接状态。观察 IPC 入口确认导航流程只有一次 codex_status，未调用 codex_check_cancel。
- Codex 0.153.4 登录、兼容缓存、强制重验与取消后禁用生成通过，没有执行真实订阅生成。
- Windows EXE / NSIS 构建成功，版本及哈希见 [窗口连接状态验收](docs/audits/codex-session-2026-09-08.md)。未重跑安装升级、Rust 全量或全应用桌面回归。


## rc.13 本地验收（2026-09-08）

该次本地验收版本为 1.1.0-rc.13，以下为历史结果。

- 前端 56 文件 / 324 项通过，TypeScript、ESLint 通过。
- AI 工作台专项验证三种窗口尺寸、独立滚动、固定操作区、模板草稿保留、Provider 设置保留、预览确认及离线结果追加后自动保存。
- Codex 0.153.4 桌面连接、真实登录识别、兼容检查、缓存与取消检查通过，未调用真实订阅生成。
- Windows EXE 与 NSIS 构建、截图及校验值见 [rc.13 验收记录](docs/audits/ai-workbench-2026-09-08.md)。本轮未重跑 Rust 全量测试、全应用桌面回归或安装升级；rc.12 的 Rust 结果仅作为历史记录。

<a id="rc11-validation"></a>

## rc.11 验收（2026-09-08）

该次验收版本为 1.1.0-rc.11。以下历史章节只代表各版本当时的结果。
...
实际 release EXE 完整桌面流程及新增修复回归通过，66 张截图涵盖三种窗口尺寸、最大侧栏、滚动详情、深浅主题与导出弹窗。公开记录含 10 项问题映射、依赖警告及产物 SHA-256，原始日志和截图保留在本机归档，见 [rc.6 验收记录](docs/audits/rc6/VALIDATION.md)。

本轮未重新通过真实 Codex 订阅验收：当前自动发现 CLI 被 0.149.1 版本门禁拒绝，未进入真实生成；不把历史结果计作 rc.6 通过。其余未覆盖范围在验收记录中单列。

以下均为历史版本证据。

<a id="rc5-validation"></a>

## rc.5 历史验收（2026-09-06）

前端 40 文件 / 264 项、Rust 94 项常规测试通过；5 项显式 CLI/订阅与容量测试另行执行全部通过。新构建 NSIS 安装版完整桌面流程、三种视口截图、原生关闭失败保护、锁定和重试保存退出通过。当前结果、依赖警告、产物追溯和未覆盖范围见 [rc.5 验收记录](docs/audits/rc5/VALIDATION.md)。

以下保留各历史版本原始测试记录，不作为当前版本的新增验收。
...
## V1.1.0-rc.3 当前验收（本地验收完成，源码基线 CI 通过）
...
| 发布 | rc.3 tag、Release 与资产 | [v1.1.0-rc.3](https://github.com/Gunanzhao/NovelForge/releases/tag/v1.1.0-rc.3)；Windows 安装包及 SHA256SUMS.txt |
