...
pub fn get_statistics(input: StatisticsInput) -> Result<Stats, String> {
    ...
    let mut statement = connection
        .prepare("SELECT created_at, delta_words FROM activity")
    ...
    let average_daily_words = daily_totals
        .values()
        .sum::<u64>()
    ...
    {
        if previous.is_some_and(|last| date == last + Duration::days(1)) {
            run += 1;
        } else {
            run = 1;
        }
    ...
    let daily = (0..30)
        ...
        .collect();
    chapter_stats.sort_by(|left, right| {
        right
            .words
            .cmp(&left.words)
            .then_with(|| left.title.cmp(&right.title))
    });
    let current_node = input
        .current_node_id
        .as_deref()
        .and_then(|id| nodes.iter().find(|node| node.id == id));
    let current_chapter_words = current_node
        .filter(|node| node.kind == "chapter")
        .and_then(|node| word_counts.get(&node.id).copied())
        .unwrap_or(0);
    ...
    if let Some(node) = current_node {
        ...
        while let Some(parent_id) = parent {
            if let Some(parent_node) = nodes.iter().find(|candidate| candidate.id == parent_id) {
                if parent_node.kind == "volume" {
                    current_volume_id = Some(parent_node.id.clone());
                    break;
                }
    ...
    let current_volume_words = current_volume_id
        .map(|volume_id| {
            nodes
                .iter()
                .filter(|node| {
                ...
                .map(|node| word_counts.get(&node.id).copied().unwrap_or(0))
                .sum()
        })
        .unwrap_or(0);
    Ok(Stats {
