        ...
        if include_title {
            if format == "markdown" {
                ...
                output.push_str(&format!(
                    "\n{}\n{}\n\n",
                    node.title,
                    "=".repeat(node.title.chars().count().max(3))
                ));
            }
        }
        if node.kind != "volume" {
            let path = storage::safe_relative(root, &node.file_path).map_err(|error| {
                format!(
                    "导出章节“{}”路径无效（{}）：{}",
                    node.title, node.file_path, error
                )
            })?;
...
fn export_list_marker(line: &str) -> Option<(bool, Option<bool>, String)> {
    ...
    if let Some((end, _)) = digits.last() {
        ...
        if let Some(marker) = trimmed[marker_end..].chars().next() {
            if (marker == '.' || marker == ')')
                && trimmed[marker_end + marker.len_utf8()..].starts_with(' ')
            {
                return Some((
                    true,
                    None,
                    trimmed[marker_end + marker.len_utf8() + 1..].to_string(),
                ));
            }
        }
    }
...
fn export_is_block_start(line: &str) -> bool {
    ...
    !trimmed.is_empty()
        && (heading_level(trimmed).is_some()
            ...
            || export_footnote_definition(trimmed).is_some())
}
...
fn parse_export_link(source: &str, image: bool) -> Option<(usize, String, String)> {
    ...
    Some((
        consumed,
        source[label_start..label_start + close].to_string(),
        source[url_start..url_start + url_end].to_string(),
    ))
}
...
fn parse_export_inline(source: &str) -> Vec<ExportInline> {
    ...
    while offset < source.len() {
        ...
        if let Some(stripped) = rest.strip_prefix("[[") {
            if let Some(end) = stripped.find("]]") {
                let target = stripped[..end].trim().to_string();
                if !target.is_empty() {
        ...
        if let Some((consumed, label, href)) = parse_export_link(rest, true) {
            result.push(ExportInline::Image {
                alt: label,
                src: href,
            });
            offset += consumed;
        ...
        if let Some((consumed, label, href)) = parse_export_link(rest, false) {
            result.push(ExportInline::Link {
                label: parse_export_inline(&label),
                href,
            });
            offset += consumed;
            continue;
...
fn parse_export_document(markdown: &str) -> ExportDocument {
    ...
    while index < lines.len() {
        ...
        if let Some((id, first_line)) = export_footnote_definition(trimmed) {
            ...
            blocks.push(ExportBlock::FootnoteDefinition {
                id,
                content: parse_export_inline(&definition),
            });
            continue;
        }
        ...
        if let Some(level) = heading_level(trimmed) {
            blocks.push(ExportBlock::Heading {
                level: level.min(6),
                content: parse_export_inline(trimmed[level + 1..].trim()),
            });
            index += 1;
            continue;
        }
        ...
        if let Some((ordered, checked, content)) = export_list_marker(line) {
            let mut items = vec![ExportListItem {
                content: parse_export_inline(&content),
                checked,
            }];
            index += 1;
            while index < lines.len() {
                let Some((next_ordered, next_checked, next_content)) =
                    export_list_marker(lines[index])
                else {
                    break;
                };
                ...
                items.push(ExportListItem {
                    content: parse_export_inline(&next_content),
        ...
        if index + 1 < lines.len()
            && export_table_cells(line).is_some()
            && export_is_table_separator(lines[index + 1])
        {
            let headers = export_table_cells(line)
                .unwrap_or_default()
                .into_iter()
                .map(|cell| parse_export_inline(&cell))
                .collect::<Vec<_>>();
            index += 2;
            let mut rows = Vec::new();
            while index < lines.len() {
                let Some(cells) = export_table_cells(lines[index]) else {
                    break;
                };
                rows.push(
                    cells
        ...
        while index < lines.len()
            ...
            && !export_is_block_start(lines[index])
        {
            if index + 1 < lines.len()
                && export_table_cells(lines[index]).is_some()
                && export_is_table_separator(lines[index + 1])
            {
                break;
            }
        ...
        blocks.push(ExportBlock::Paragraph(parse_export_inline(
            &paragraph.join("\n"),
        )));
    }
...
fn export_plain_inlines(inlines: &[ExportInline]) -> String {
    inlines
        ...
        .map(|inline| match inline {
            ...
            | ExportInline::Strike(children) => export_plain_inlines(children),
...
fn export_plain_text(document: &ExportDocument) -> String {
    ...
    for block in &document.blocks {
        match block {
            ExportBlock::Heading { content, .. }
            | ExportBlock::Paragraph(content)
            | ExportBlock::Quote(content) => lines.push(export_plain_inlines(content)),
            ExportBlock::List { items, .. } => {
                lines.extend(items.iter().map(|item| export_plain_inlines(&item.content)))
            }
            ...
            ExportBlock::Table { headers, rows } => {
                lines.push(
                    headers
                        .iter()
                        .map(|cell| export_plain_inlines(cell))
                        .collect::<Vec<_>>()
                        .join("\t"),
                );
            ...
            ExportBlock::FootnoteDefinition { id, content } => {
                lines.push(format!("[^{}]: {}", id, export_plain_inlines(content)))
            }
        }
    }
    lines
...
fn export_scope_nodes(
    ...
    match scope {
        ...
        "volume" => {
            let volume_path = input
                .volume_path
                .as_deref()
                .ok_or_else(|| "指定卷导出需要卷路径".to_string())?;
            let volume = nodes
                .iter()
                .find(|node| node.kind == "volume" && node.file_path == volume_path)
        ...
        "chapters" => {
            let ids = input
                .node_ids
                .as_deref()
                .ok_or_else(|| "指定章节导出需要章节 ID".to_string())?;
            for id in ids {
                let node = nodes
                    .iter()
...
fn export_html_inlines(inlines: &[ExportInline]) -> String {
    inlines
        ...
        .map(|inline| match inline {
            ...
            ExportInline::Strong(children) => {
                format!("<strong>{}</strong>", export_html_inlines(children))
            }
            ExportInline::Emphasis(children) => {
                format!("<em>{}</em>", export_html_inlines(children))
            }
            ExportInline::Strike(children) => {
                format!("<del>{}</del>", export_html_inlines(children))
            }
            ...
            ExportInline::Wiki(target) => format!(
                "<a class=\"wiki-link\" data-wiki-target=\"{}\" href=\"#wiki-{}\">{}</a>",
                xml_escape(target),
                xml_escape(&target.replace(' ', "-")),
                xml_escape(target)
            ),
...
fn export_html_fragment(document: &ExportDocument, include_toc: bool) -> (String, String) {
    ...
    for block in &document.blocks {
        match block {
            ...
            ExportBlock::Paragraph(content) => {
                body.push_str(&format!("<p>{}</p>", export_html_inlines(content)))
            }
            ExportBlock::Quote(content) => body.push_str(&format!(
                "<blockquote>{}</blockquote>",
                export_html_inlines(content)
            )),
            ExportBlock::List { ordered, items } => {
                ...
                for item in items {
                    ...
                    body.push_str(&format!(
                        "<li>{}{}</li>",
                        checkbox,
                        export_html_inlines(&item.content)
                    ));
                }
                body.push_str(&format!("</{}>", tag));
            }
            ExportBlock::CodeBlock(content) => {
                body.push_str(&format!("<pre><code>{}</code></pre>", xml_escape(content)))
            }
            ExportBlock::HorizontalRule => body.push_str("<hr/>"),
            ExportBlock::Table { headers, rows } => {
                body.push_str("<table><thead><tr>");
                for cell in headers {
                    body.push_str(&format!("<th>{}</th>", export_html_inlines(cell)));
                }
                body.push_str("</tr></thead><tbody>");
                for row in rows {
                    body.push_str("<tr>");
                    for cell in row {
                        body.push_str(&format!("<td>{}</td>", export_html_inlines(cell)));
                    }
                    body.push_str("</tr>");
                }
    ...
    if !footnotes.is_empty() {
        body.push_str(&format!(
            "<section class=\"footnotes\"><h2>脚注</h2><ol>{}</ol></section>",
            footnotes.join("")
        ));
    }
    (body, toc)
    ...
    for (index, line) in text.split('\n').enumerate() {
        ...
        value.push_str(&format!(
            ...
            if run_properties.is_empty() {
                ...
                format!("<w:rPr>{}</w:rPr>", run_properties)
            },
            xml_escape(line)
        ));
    }
...
fn export_docx_inlines_with_properties(inlines: &[ExportInline], inherited: &str) -> String {
    ...
    for inline in inlines {
        match inline {
            ...
            ExportInline::Strong(children) => result.push_str(
                &export_docx_inlines_with_properties(children, &(inherited.to_string() + "<w:b/>")),
            ),
            ExportInline::Emphasis(children) => result.push_str(
                &export_docx_inlines_with_properties(children, &(inherited.to_string() + "<w:i/>")),
            ),
            ExportInline::Strike(children) => {
                result.push_str(&export_docx_inlines_with_properties(
                    children,
                    &(inherited.to_string() + "<w:strike/>"),
                ))
            }
            ExportInline::Code(text) => result.push_str(&docx_run(
                text,
                &(inherited.to_string() + "<w:rStyle w:val=\"CodeChar\"/>"),
            )),
            ExportInline::Link { label, .. } => {
                result.push_str(&export_docx_inlines_with_properties(
                    label,
                    &(inherited.to_string() + "<w:color w:val=\"0563C1\"/><w:u w:val=\"single\"/>"),
                ))
            }
            ExportInline::Image { alt, .. } => result.push_str(&docx_run(
                alt,
                &(inherited.to_string() + "<w:color w:val=\"666666\"/>"),
            )),
            ExportInline::Wiki(target) => result.push_str(&docx_run(
                target,
                &(inherited.to_string() + "<w:color w:val=\"7030A0\"/><w:u w:val=\"single\"/>"),
            )),
            ExportInline::FootnoteReference(id) => {
                result.push_str(&docx_run(&format!("[^{}]", id), inherited))
            }
        }
    }
    result
...
fn export_docx_table(table: &ExportBlock) -> String {
    let ExportBlock::Table { headers, rows } = table else {
        return String::new();
    };
    ...
    for cell in headers {
        result.push_str(&format!(
            "<w:tc><w:p>{}</w:p></w:tc>",
            export_docx_inlines_with_properties(cell, "<w:b/>")
        ));
    }
    result.push_str("</w:tr>");
    for row in rows {
        ...
        for cell in row {
            result.push_str(&format!(
                "<w:tc><w:p>{}</w:p></w:tc>",
                export_docx_inlines_with_properties(cell, "")
            ));
        }
        result.push_str("</w:tr>");
    }
...
fn export_docx_xml_from_document(document: &ExportDocument, cover: Option<&ExportCover>) -> String {
    let mut body = String::new();
    if cover.is_some() {
        body.push_str(&docx_cover_drawing());
    }
    for block in &document.blocks {
        match block {
    ...
    format!(
        r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><w:body>{}<w:sectPr><w:pgSz w:w="11906" w:h="16838"/><w:pgMar w:top="1440" w:right="1440" w:bottom="1440" w:left="1440"/></w:sectPr></w:body></w:document>"#,
        body
    )
}
...
fn zip_document_with_binary(
    ...
    for (name, content) in files {
        let method = if stored.iter().any(|item| item == name) {
        ...
        writer
            ...
            .map_err(|error| format!("创建压缩文件失败：{}", error))?;
        writer
    ...
    writer
        ...
        .map_err(|error| format!("完成压缩文件失败：{}", error))
}
...
fn export_base64(bytes: &[u8]) -> String {
    ...
    while index < bytes.len() {
        ...
        result.push(if index + 1 < bytes.len() {
            ALPHABET[((combined >> 6) & 63) as usize] as char
        } else {
            '='
        });
        result.push(if index + 2 < bytes.len() {
...
fn docx_bytes(markdown: &str, cover: Option<&ExportCover>) -> Result<Vec<u8>, String> {
    ...
    let binary = cover
        .map(|asset| {
            vec![(
                cover_name.as_deref().unwrap_or("word/media/cover.bin"),
    ...
    groups
        ...
        .map(|(index, blocks)| {
            let label = blocks
                .iter()
                .find_map(|block| {
                    if let ExportBlock::Heading { content, .. } = block {
                        Some(export_plain_inlines(content))
                    } else {
                        None
                    }
                })
...
fn epub_xhtml_document(
    document: &ExportDocument,
    cover: Option<&ExportCover>,
    title: &str,
) -> String {
    let (body, _) = export_html_fragment(document, false);
    let cover_markup = cover
        .map(|asset| {
            format!(
                "<p class=\"cover\"><img src=\"images/{}\" alt=\"封面\" /></p>",
                xml_escape(&asset.file_name)
            )
        })
        .unwrap_or_default();
    format!(
        r#"<?xml version="1.0" encoding="utf-8"?><!DOCTYPE html><html xmlns="http://www.w3.org/1999/xhtml"><head><meta charset="utf-8"/><title>{}</title><style>body{{font-family:serif;line-height:1.8;margin:5%;}}h1,h2,h3{{line-height:1.3;}}.cover{{text-align:center}}.cover img{{max-width:100%;max-height:520px}}</style></head><body>{}{}</body></html>"#,
        xml_escape(title),
        cover_markup,
        body
    )
...
fn epub_bytes(
    ...
    for (file_name, part, label) in &parts {
        let level = part
            ...
            .iter()
            .find_map(|block| {
                if let ExportBlock::Heading { level, .. } = block {
                    Some(*level)
                } else {
                    None
                }
        ...
        nav_entries.push_str(&format!(
            "<li class=\"toc-level-{}\"><a href=\"{}#heading-1\">{}</a></li>",
            level,
    ...
    let nav = format!(
        r#"<?xml version="1.0" encoding="utf-8"?><!DOCTYPE html><html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops"><head><title>{}</title></head><body><nav epub:type="toc" id="toc"><h1>目录</h1><ol>{}</ol></nav></body></html>"#,
        xml_escape(title),
        nav_entries
    );
    ...
    let cover_metadata = cover
        .map(|_| "<meta name=\"cover\" content=\"cover-image\"/>")
        .unwrap_or_default();
    let part_manifest = parts
        .iter()
        .enumerate()
    ...
    let binary = cover
        .map(|asset| {
            vec![(
                cover_name.as_deref().unwrap_or("OEBPS/images/cover.bin"),
                asset.bytes.as_slice(),
            )]
        })
        .unwrap_or_default();
...
fn pdf_bytes_legacy(text: &str, cover: Option<&ExportCover>) -> Vec<u8> {
    let jpeg = cover
        .and_then(|asset| pdf_jpeg_dimensions(&asset.bytes).map(|dimensions| (dimensions, asset)));
    let mut lines = Vec::new();
    ...
    for (page_index, chunk) in page_lines.into_iter().enumerate() {
        ...
        stream.push_str("ET\n");
        let image_resources = if jpeg.is_some() {
            " /XObject << /Im1 IMGREF >>"
        } else {
            ""
        };
    ...
    for (index, object) in objects.iter().enumerate() {
        ...
        let number = index + 1;
        let image_reference = image_number
            .map(|number| format!("{} 0 R", number))
            .unwrap_or_default();
        pdf.extend_from_slice(
            format!(
                ...
                number,
                object
                    .replace("FONTREF", &format!("{} 0 R", font_number))
                    .replace("DESCREF", &format!("{} 0 R", descendant_number))
                    .replace("IMGREF", &image_reference)
            )
            .as_bytes(),
        );
    }
...
fn pdf_bytes_embedded(text: &str, cover: Option<&ExportCover>, font: &ParsedFont) -> Vec<u8> {
    ...
    let cover_image =
        cover.and_then(|asset| RawImage::decode_from_bytes(&asset.bytes, &mut image_warnings).ok());
    ...
    for (page_index, chunk) in chunks.iter().enumerate() {
        ...
        operations.push(Op::SetTextCursor {
            pos: Point::new(Pt(50.0).into(), Pt(790.0).into()),
        });
    ...
    let (extension, bytes) = match format {
        ...
        "html" => (
            ...
            html_bytes(
                ...
                &title,
                &author,
                input.include_toc.unwrap_or(true),
                cover.as_ref(),
            ),
        ),
        "docx" => (
            "docx".to_string(),
            docx_bytes(&markdown_document, cover.as_ref())?,
        ),
        "epub" => (
            "epub".to_string(),
            epub_bytes(&markdown_document, &title, &author, cover.as_ref())?,
        ),
        "pdf" => (
            "pdf".to_string(),
            pdf_bytes(
                &pdf_plain_text(&format!("{}\n作者：{}\n\n{}", title, author, markdown)),
                cover.as_ref(),
            ),
        ),
        _ => unreachable!(),
...
mod tests {
    ...
    fn export_rejects_unreadable_or_invalid_chapters_in_every_format() {
        let root =
            std::env::temp_dir().join(format!("novelforge-export-errors-{}", storage::new_id()));
        let project_path = root.to_string_lossy().to_string();
        let project = create_project(ProjectInput {
            path: project_path.clone(),
            title: "导出回归".to_string(),
            author: String::new(),
        ...
        let chapter = project
            ...
            .unwrap();
        let chapter_path = root.join(&chapter.file_path);
        let original = fs::read(&chapter_path).unwrap();
        let export = |format: &str| {
            export_project(
                serde_json::from_value(serde_json::json!({
                    "projectPath": project_path, "format": format,
                }))
                .unwrap(),
            )
        };
        let successful = export("markdown").unwrap();
        assert!(fs::read_to_string(&successful)
            .unwrap()
            .contains("从这里开始你的故事。"));
        fs::remove_file(successful).unwrap();
        for failure in ["missing", "invalid-utf8", "invalid-path"] {
            if failure == "missing" {
                fs::remove_file(&chapter_path).unwrap();
            } else if failure == "invalid-utf8" {
                fs::write(&chapter_path, [0xff, 0xfe]).unwrap();
            } else {
                fs::write(&chapter_path, &original).unwrap();
                let (_, connection) = project_connection(&project_path).unwrap();
                connection
                    .execute(
                        "UPDATE nodes SET file_path = '../outside.md' WHERE id = ?1",
                        params![chapter.id],
                    )
                    .unwrap();
            }
            for format in ["markdown", "txt", "html", "docx", "epub", "pdf"] {
                let error = export(format).unwrap_err();
                assert!(
                    error.contains(&chapter.title),
                    "{failure}/{format}: {error}"
                );
                assert!(error.contains(if failure == "invalid-path" {
                    "路径无效"
                } else {
                    "读取导出章节"
                }));
            ...
            assert_eq!(
                fs::read_dir(root.join(".novelforge/exports"))
                    .unwrap()
                    .count(),
                0
            );
