...
fn real_command_workflow_persists_markdown_and_recoverable_trash() {
    ...
    let trash_id = trash
        .iter()
        .find(|item| item.ref_id == chapter.id)
        .expect("trash item")
        .id
        .clone();
    ...
    assert!(super::commands::list_trash(project_path.clone())
...
fn create_and_rename_nodes_roll_back_files_on_database_failure() {
    ...
    assert!(
        super::commands::rename_node(super::models::RenameNodeInput {
            project_path: project_path.clone(),
            node_id: chapter.id.clone(),
            title: "失败标题".to_string(),
        })
        .is_err()
    );
...
fn entity_upsert_rolls_back_markdown_mirror_on_database_failure() {
    ...
    let created = super::commands::upsert_entity(super::models::EntityInput {
        ...
        id: None,
        title: "林月".to_string(),
        content: serde_json::json!({"identity": "旧身份"}),
        tags: vec!["主角".to_string()],
    })
    ...
    assert!(super::commands::upsert_entity(super::models::EntityInput {
        ...
        title: "林月".to_string(),
        content: serde_json::json!({"identity": "不应写入"}),
        tags: vec!["回滚".to_string()],
    })
    .is_err());
    assert_eq!(
        fs::read_to_string(&mirror).expect("rolled back mirror"),
        original
    );
...
fn opening_corrupt_database_rebuilds_markdown_tree() {
    ...
    super::commands::save_document(super::models::SaveDocumentInput {
        project_path: project_path.clone(),
        node_id: chapter.id,
        content: "# 第一章\n\n数据库损坏后仍应可读。".to_string(),
        reason: "恢复前".to_string(),
    })
    .expect("save document");
    super::commands::upsert_entity(super::models::EntityInput {
    ...
    fs::write(
        root.join("project/.novelforge/database.sqlite"),
        b"not a sqlite database",
    )
    .expect("corrupt database");

    let reopened =
        super::commands::open_project(project_path.clone()).expect("open recovered project");
    let recovered = reopened
        .nodes
        .iter()
        .find(|node| node.kind == "chapter")
    ...
    let document = super::commands::get_document(super::models::NodeActionInput {
        project_path: project_path.clone(),
        node_id: recovered.id,
    })
    .expect("read recovered markdown");
    assert!(document.content.contains("数据库损坏后仍应可读"));
    let recovered_entity = reopened
        .entities
        .iter()
        .find(|entity| entity.title == "林月")
        .expect("recovered character");
    ...
    let backups = fs::read_dir(root.join("project/.novelforge"))
...
fn recovery_rejects_directory_link_outside_project_before_quarantine() {
    ...
    let error =
        super::commands::open_project(project_path).expect_err("outside link must abort recovery");
...
fn corrupt_database_preserves_stable_ids_and_relationships() {
    ...
    let relationship = relationship_data
        .entities
        .iter()
        .find(|entity| entity.kind == "relationship")
        .expect("relationship")
        .clone();
    ...
    let continent = continent_data
        .entities
        .iter()
        .find(|entity| entity.title == "大陆")
        .expect("continent")
        .clone();
    ...
    let country = country_data
        .entities
        .iter()
        .find(|entity| entity.title == "国家")
        .expect("country")
        .clone();
    ...
    let outline = outline_data
        .entities
        .iter()
        .find(|entity| entity.kind == "outline")
        .expect("outline")
        .clone();
    ...
    let entity_raw = fs::read_to_string(root.join("project").join(&person_a.file_path))
        .expect("read entity mirror");
    assert!(entity_raw.contains(&format!("novelforgeId: {}", person_a.id)));
    fs::write(root.join("project/.novelforge/database.sqlite"), b"corrupt")
        .expect("corrupt database");

    let reopened =
        super::commands::open_project(project_path.clone()).expect("reopen recovered project");
    let recovered_volume = reopened
        .nodes
        .iter()
        .find(|node| node.kind == "volume")
        .expect("recovered volume");
    let recovered_chapter = reopened
        .nodes
        .iter()
        .find(|node| node.kind == "chapter")
        .expect("recovered chapter");
    let recovered_section = reopened
        .nodes
        .iter()
        .find(|node| node.kind == "section")
        .expect("recovered section");
    ...
    assert_eq!(
        recovered_section.parent_id.as_deref(),
        Some(chapter.id.as_str())
    );
    let recovered_relationship = reopened
        .entities
        .iter()
        .find(|entity| entity.id == relationship.id)
    ...
    assert_eq!(
        recovered_relationship
            ...
            .get("toId")
            .and_then(serde_json::Value::as_str),
        Some(person_b.id.as_str())
    );
    let recovered_country = reopened
        .entities
        .iter()
        .find(|entity| entity.id == country.id)
        .expect("recovered country");
    let recovered_city = reopened
        .entities
        .iter()
        .find(|entity| entity.id == city.id)
        .expect("recovered city");
    assert_eq!(
        recovered_country
            .content
            .get("parentId")
            .and_then(serde_json::Value::as_str),
        Some(continent.id.as_str())
    );
    assert_eq!(
        recovered_city
            .content
            .get("parentId")
            .and_then(serde_json::Value::as_str),
        Some(country.id.as_str())
    );
    let recovered_outline = reopened
        .entities
        .iter()
        .find(|entity| entity.id == outline.id)
        .expect("recovered outline");
    let recovered_scene = reopened
        .entities
        .iter()
        .find(|entity| entity.id == scene.id)
...
fn corrupt_database_keeps_legacy_markdown_recoverable_and_warns() {
    ...
    fs::write(
        &chapter_path,
        storage::strip_markdown_frontmatter(&chapter_raw),
    )
    .expect("strip chapter metadata");
    let entity_path = root.join("project").join(&entity.file_path);
    let entity_raw = fs::read_to_string(&entity_path).expect("read entity");
    fs::write(
        &entity_path,
        storage::strip_markdown_frontmatter(&entity_raw),
    )
    .expect("strip entity metadata");
    fs::write(
        root.join("project/.novelforge/database.sqlite"),
        b"legacy corrupt",
    )
    ...
    let reopened =
        super::commands::open_project(project_path.clone()).expect("reopen legacy project");
    let recovered_chapter = reopened
        .nodes
        .iter()
        .find(|node| node.kind == "chapter")
...
fn move_and_copy_nodes_keep_markdown_files_and_tree_paths_in_sync() {
    ...
    .expect("create project");
    let first_volume = created
        .nodes
        .iter()
        .find(|node| node.kind == "volume")
        .expect("first volume")
        .clone();
    let first_chapter = created
        .nodes
        .iter()
        .find(|node| node.kind == "chapter")
    ...
    let second_volume = second_volume_data
        .nodes
        .iter()
        .find(|node| node.title == "第二卷")
    ...
    let section_data = super::commands::create_node(super::models::NodeInput {
        ...
        kind: "section".to_string(),
        title: "开场".to_string(),
        parent_id: Some(first_chapter.id.clone()),
    })
    .expect("create section");
    let section = section_data
        .nodes
        .iter()
        .find(|node| node.title == "开场")
        .expect("section")
        .clone();
    super::commands::save_document(super::models::SaveDocumentInput {
        project_path: project_path.clone(),
        node_id: first_chapter.id.clone(),
        content: "# 第一章\n\n移动后仍然可读".to_string(),
        reason: "移动复制测试".to_string(),
    })
    .expect("save chapter");
    let moved = super::commands::move_node(super::models::MoveNodeInput {
        project_path: project_path.clone(),
        node_id: first_chapter.id.clone(),
        target_parent_id: Some(second_volume.id.clone()),
        target_order_index: None,
    })
    .expect("move chapter");
    let moved_chapter = moved
        .nodes
        .iter()
        .find(|node| node.id == first_chapter.id)
        .expect("moved chapter");
    assert_eq!(
        moved_chapter.parent_id.as_deref(),
        Some(second_volume.id.as_str())
    );
    assert!(moved_chapter
        .file_path
        .starts_with(&second_volume.file_path));
    let moved_section = moved
        .nodes
        .iter()
        .find(|node| node.id == section.id)
        .expect("moved section");
    assert!(moved_section
        .file_path
        .starts_with(moved_chapter.file_path.trim_end_matches(".md")));
    ...
    assert_eq!(
        storage::strip_markdown_frontmatter(&moved_content),
        "# 第一章\n\n移动后仍然可读"
    );
    ...
    let copied = super::commands::copy_node(super::models::CopyNodeInput {
        ...
        title: Some("第一章副本".to_string()),
    })
    .expect("copy chapter");
    let copied_chapter = copied
        .nodes
        .iter()
        .find(|node| node.title == "第一章副本")
        .expect("copied chapter");
    assert_ne!(copied_chapter.id, first_chapter.id);
    assert_eq!(
        copied_chapter.parent_id.as_deref(),
        Some(first_volume.id.as_str())
    );
    let copied_content = fs::read_to_string(root.join("project").join(&copied_chapter.file_path))
        .expect("copied file");
    assert_eq!(
        storage::strip_markdown_frontmatter(&copied_content),
        "# 第一章副本\n\n移动后仍然可读"
    );
    let copied_section_id = copied
        .nodes
        .iter()
        .find(|node| node.parent_id.as_deref() == Some(copied_chapter.id.as_str()))
...
fn trash_path_reuse_allocates_unique_path_and_restores_sidecar() {
    ...
    let trash_item = trash
        .iter()
        .find(|item| item.ref_id == second.id)
        .expect("second trash item")
        .clone();
    ...
    let replacement = new_data
        .nodes
        .iter()
        .find(|node| node.title == "新章节")
        .expect("replacement")
        .clone();
    ...
    assert_eq!(
        restored_section.parent_id.as_deref(),
        Some(second.id.as_str())
    );
    assert_eq!(
        storage::strip_markdown_frontmatter(
            &fs::read_to_string(root.join("project").join(&restored_chapter.file_path))
                .expect("restored chapter file")
        ),
        "# 第二章\n\n旧章节正文"
    );
    assert_eq!(
        storage::strip_markdown_frontmatter(
            &fs::read_to_string(root.join("project").join(&restored_section.file_path))
                .expect("restored section file")
        ),
        "# 第二章·节\n\n小节正文"
    );
    let sibling_orders: Vec<i64> = restored
        .nodes
        .iter()
    ...
    let restored_trash =
        super::commands::list_trash(project_path.clone()).expect("list restored trash");
    let restored_item = restored_trash
        .iter()
        .find(|item| item.ref_id == second.id)
        .expect("restored trash item")
        .clone();
...
fn trash_path_must_stay_inside_project() {
    ...
    let outside = root
        ...
        .join(format!("novelforge-outside-{}", storage::new_id()));
    fs::create_dir_all(&outside).expect("outside directory");
...
fn save_failure_restores_original_and_keeps_recovery_file() {
    ...
    let result = super::commands::save_document(super::models::SaveDocumentInput {
        project_path: project_path.clone(),
        node_id: chapter.id.clone(),
        content: "新正文，不应覆盖原稿".to_string(),
        reason: "触发失败".to_string(),
    });
    assert!(result.is_err());
    let original_after_failure = fs::read_to_string(root.join("project").join(&chapter.file_path))
        .expect("original remains");
    assert_eq!(
        storage::strip_markdown_frontmatter(&original_after_failure),
        "原正文"
    );
...
fn node_delete_restores_file_when_database_transaction_fails() {
    ...
    assert!(result.is_err());
    assert_eq!(
        fs::read_to_string(&chapter_path).expect("restored chapter"),
        original
    );
    let reopened = super::commands::open_project(project_path).expect("reopen project");
    let restored = reopened
...
fn entity_delete_restores_file_when_database_transaction_fails() {
    ...
    let entity = with_entity
        .entities
        .iter()
        .find(|item| item.title == "第一次停电")
    ...
    assert!(result.is_err());
    assert_eq!(
        fs::read_to_string(&entity_path).expect("restored entity"),
        original
    );
    ...
    assert!(super::commands::list_trash(project_path)
        .expect("list trash")
        .is_empty());
    let _ = fs::remove_dir_all(root);
}
...
fn attachment_import_copies_binary_and_metadata_updates_keep_bytes() {
    ...
    let imported = super::commands::import_attachment(super::models::AttachmentInput {
        ...
        description: "初始参考资料".to_string(),
    })
    .expect("import attachment");
    let attachment = imported
        .entities
        .iter()
        .find(|item| item.kind == "attachment")
    ...
    assert_eq!(
        fs::read(&destination).expect("bytes after metadata update"),
        bytes
    );
    let _ = fs::remove_dir_all(root);
}
...
fn consistency_check_reports_missing_wiki_and_broken_relationship() {
    ...
    super::commands::save_document(super::models::SaveDocumentInput {
        project_path: project_path.clone(),
        node_id: chapter.id,
        content: "正文引用[[不存在的人物]]".to_string(),
        reason: "一致性测试".to_string(),
    })
    .expect("save document");
    super::commands::upsert_entity(super::models::EntityInput {
        project_path: project_path.clone(),
        ...
        id: None,
        title: "坏关系".to_string(),
        content: serde_json::json!({"fromId": "missing-a", "toId": "missing-b", "label": "敌对"}),
        tags: vec!["人物关系".to_string()],
    })
    ...
    assert!(report
        .issues
        .iter()
        .any(|issue| issue.code == "missing-wiki"));
    assert!(report
        .issues
        .iter()
        .any(|issue| issue.code == "broken-relationship"));
...
fn consistency_check_reports_structured_character_and_timeline_conflicts() {
    ...
    let entries = [
        (
            "character",
            "林月",
            serde_json::json!({"age": 18, "ageAtChapter": {"first": 25}, "birthday": "2000-01-01", "birthDate": "2001-02-02", "gender": "女", "sex": "男", "status": "dead", "deathDate": "2024-01-15"}),
        ),
        ...
        (
            "timeline",
            "死亡事件",
            serde_json::json!({"date": "2024-01-15", "characters": "c1", "status": "dead"}),
        ),
        (
            "timeline",
            "异常活动",
            serde_json::json!({"date": "2024-02-01", "characters": "c1", "age": 30, "status": "active", "startDate": "2024-04-01", "endDate": "2024-03-01"}),
        ),
        (
            "timeline",
            "更早事件",
            serde_json::json!({"date": "2023-12-01", "characters": "c1"}),
        ),
    ];
        ...
        if let Some(object) = content.as_object_mut() {
            if object.get("characters").and_then(serde_json::Value::as_str) == Some("c1") {
                object.insert(
                    ...
                    serde_json::Value::String(character_id.clone()),
                );
            }
        }
    ...
    let report =
        super::commands::check_consistency(project_path.clone()).expect("consistency report");
...
fn consistency_check_accepts_global_chapter_numbers_across_volumes() {
    ...
    super::commands::upsert_entity(super::models::EntityInput {
        ...
        kind: "timeline".to_string(),
        id: None,
        title: "第三章事件".to_string(),
        content: serde_json::json!({"chapters": "第3章", "description": "跨卷章节引用"}),
        tags: vec!["时间线".to_string()],
    })
    ...
    assert!(!report
        .issues
        .iter()
        .any(|issue| issue.code == "missing-chapter-reference"));
    let _ = fs::remove_dir_all(root);
}
...
fn consistency_check_accepts_legacy_paid_off_foreshadowing_status() {
    ...
    super::commands::upsert_entity(super::models::EntityInput {
        ...
        id: None,
        title: "旧伏笔".to_string(),
        content: serde_json::json!({"actualPayoff": chapter.title, "status": "resolved"}),
    ...
    assert!(!report
        .issues
        .iter()
        .any(|issue| issue.code == "foreshadowing-status"));
    let _ = fs::remove_dir_all(root);
}
...
fn statistics_include_daily_series_and_chapter_breakdown() {
    ...
    super::commands::save_document(super::models::SaveDocumentInput {
        ...
        node_id: chapter.id.clone(),
        content: "# 第一章\n\n一段统计正文".repeat(20),
        reason: "统计测试".to_string(),
    })
    .expect("save document");
    let stats = super::commands::get_statistics(super::models::StatisticsInput {
        project_path,
        current_node_id: Some(chapter.id.clone()),
    })
    .expect("statistics");
...
fn export_project_writes_all_supported_formats() {
    ...
    super::commands::save_document(super::models::SaveDocumentInput {
        ...
        node_id: chapter.id,
        content: "# 第一章\n\n## 小节\n\n- **林月**走进[[雾港]]。".to_string(),
        reason: "导出测试".to_string(),
    })
    .expect("save document");

    for format in ["markdown", "txt", "html", "docx", "epub", "pdf"] {
        let output = super::commands::export_project(super::models::ExportInput {
            project_path: project_path.clone(),
        ...
        match format {
            ...
            "epub" => {
                let mut archive =
                    zip::ZipArchive::new(std::io::Cursor::new(bytes)).expect("read epub zip");
            ...
            "pdf" => {
                ...
                assert!(
                    !bytes.windows(4).any(|window| window == b"0023"),
                    "PDF must not expose Markdown heading markers"
                );
            }
            _ => unreachable!(),
        }
    }
    assert!(super::commands::export_project(super::models::ExportInput {
        project_path,
...
fn export_project_preserves_markdown_structure_and_embeds_cover() {
    ...
    let input = |format: &str| super::models::ExportInput {
        project_path: project_path.clone(),
        format: format.to_string(),
        cover_path: Some("attachments/cover.png".to_string()),
        ..Default::default()
    };
    ...
    assert!(
        html.contains("<ul>") && html.contains("<ol>") && html.contains("<input type=\"checkbox\"")
    );
    ...
    let mut docx = zip::ZipArchive::new(std::io::Cursor::new(
        fs::read(&docx_path).expect("read docx"),
    ))
    .expect("docx zip");
    ...
    assert!(
        document_xml.contains("Heading1")
    ...
    let mut epub = zip::ZipArchive::new(std::io::Cursor::new(
        fs::read(&epub_path).expect("read epub"),
    ))
    .expect("epub zip");
    let mut xhtml = String::new();
    epub.by_name("OEBPS/content.xhtml")
        .expect("content xhtml")
        .read_to_string(&mut xhtml)
        .expect("read xhtml");
    let mut nav = String::new();
    epub.by_name("OEBPS/nav.xhtml")
        .expect("nav xhtml")
        .read_to_string(&mut nav)
        .expect("read nav");
    assert!(
        xhtml.contains("<strong>粗体</strong>")
            && xhtml.contains("<table>")
            && xhtml.contains("class=\"wiki-link\"")
    );
    assert!(
        nav.contains("chapter-001.xhtml#heading-1")
            && epub.by_name("OEBPS/images/cover.png").is_ok()
    );
    let _ = fs::remove_dir_all(root);
}
...
fn ai_endpoint_normalization_rejects_invalid_urls() {
    assert_eq!(
        super::commands::normalize_ai_endpoint("http://127.0.0.1:1234/v1").expect("base endpoint"),
        "http://127.0.0.1:1234/v1/chat/completions"
    );
    assert_eq!(
        super::commands::normalize_ai_endpoint("https://api.example.com/v1/chat/completions/")
            .expect("completion endpoint"),
        "https://api.example.com/v1/chat/completions"
    );
    assert_eq!(
        super::commands::normalize_ai_endpoint("https://api.example.com/base?api-version=1")
            .expect("query endpoint"),
        "https://api.example.com/base/v1/chat/completions?api-version=1"
    );
...
fn large_project_acceptance_handles_1000_chapters_and_one_million_characters() {
    ...
    super::commands::save_document(super::models::SaveDocumentInput {
        project_path: project_path.clone(),
        node_id: first_chapter,
        content: "字".repeat(1000),
        reason: "大规模验收".to_string(),
    })
    ...
    let transaction = connection
        .transaction()
        .expect("start large-project seed transaction");
    ...
    for volume_number in 2..=10 {
        ...
        let readme = storage::safe_relative(&project_root, &format!("{}/README.md", file_path))
            .expect("safe volume mirror");
        storage::atomic_write(
            &readme,
            storage::markdown_volume(&id, &title, "not-started", &timestamp, &timestamp).as_bytes(),
        )
    ...
    for chapter_number in 2..=1000 {
        ...
        let file_path = format!(
            "manuscript/volume_{:03}/chapter_{:03}.md",
            volume_index + 1,
            chapter_in_volume
        );
        let content = format!("# {}\n\n{}", title, "字".repeat(1000));
        ...
        storage::index_record(&transaction, &id, "chapter", &title, &content, &file_path)
            .expect("index large chapter");
        ...
        storage::atomic_write(
            &storage::safe_relative(&project_root, &file_path).expect("safe chapter mirror"),
            mirror.as_bytes(),
        )
    ...
    for (kind, directory, title_prefix, count) in [
        ...
        for index in 0..count {
            ...
            let content = match kind {
                ...
                "timeline" => {
                    serde_json::json!({"date": format!("第{}日", index + 1), "chapters": format!("第{}章", (index % 1000) + 1), "description": "大规模验收事件"})
                }
                _ => {
                    serde_json::json!({"status": "planned", "plannedPayoff": format!("第{}章", index + 10), "description": "大规模验收伏笔"})
                }
            };
            ...
            storage::index_record(
                ...
                kind,
                &title,
                &storage::markdown_entity(&title, &content, &tags),
                &file_path,
            )
            .expect("index large entity");
            let mirror = storage::markdown_entity_with_metadata(
                &id, kind, &timestamp, &timestamp, &title, &content, &tags,
            );
            storage::atomic_write(
                &storage::safe_relative(&project_root, &file_path).expect("safe entity mirror"),
                mirror.as_bytes(),
            )
    ...
    transaction
        .commit()
        .expect("commit large-project seed transaction");
    ...
    assert_eq!(
        data.entities
            .iter()
            .filter(|entity| entity.kind == "world")
            .count(),
        200
    );
    assert_eq!(
    ...
    assert_eq!(
        data.entities
            .iter()
            .filter(|entity| entity.kind == "foreshadowing")
            .count(),
        100
    );
    let search = super::commands::search_project(super::models::SearchInput {
        project_path: project_path.clone(),
        query: "世界观099".to_string(),
        kind: Some("world".to_string()),
        scope: None,
        node_id: None,
        volume_path: None,
        tag: Some("大规模".to_string()),
        case_sensitive: None,
    })
...
fn v1_1_large_auxiliary_data_acceptance() {
    ...
    let transaction = connection
        .transaction()
        .expect("start V1.1 seed transaction");
    ...
    for (kind, directory, title_prefix, count) in specs {
        for index in 0..count {
            ...
            let content = match kind {
                ...
                "inbox" => {
                    serde_json::json!({"content": "V1.1 大规模灵感内容", "processed": false})
                }
                "prompt-preset" => {
                    serde_json::json!({"action": "analyze", "prompt": "分析：{{currentChapter}}", "defaultContexts": []})
                }
                _ => serde_json::json!({
    ...
    let data =
        super::commands::open_project(project_path.clone()).expect("open V1.1 large project");
    assert_eq!(
        data.entities
            .iter()
            .filter(|entity| entity.kind == "story-arc")
            .count(),
        50
    );
    assert_eq!(
        data.entities
            .iter()
            .filter(|entity| entity.kind == "inbox")
            .count(),
        500
    );
    assert_eq!(
        data.entities
            .iter()
            .filter(|entity| entity.kind == "prompt-preset")
            .count(),
        100
    );
    assert_eq!(
        data.entities
            .iter()
            .filter(|entity| entity.kind == "chapter-checklist")
            .count(),
        1000
    );
    for (query, kind) in [
...
fn restore_trash_keeps_quarantined_entity_when_destination_exists() {
    ...
    let entity = with_entity
        .entities
        .iter()
        .find(|item| item.title == "缺失的钟声")
        .expect("foreshadowing entity")
        .clone();
    ...
    let trash_item = trash
        .iter()
        .find(|item| item.ref_id == entity.id)
        .expect("entity trash item")
        .clone();
    ...
    let trash_path = storage::safe_trash_path(&root.join("project"), &trash_item.trash_path)
        .expect("trash path");
    ...
    assert!(result.is_err());
    assert_eq!(
        fs::read_to_string(&original_path).expect("collision remains"),
        "用户新建的同名文件"
    );
    ...
    assert!(super::commands::list_trash(project_path)
        ...
        .iter()
        .any(|item| item.ref_id == entity.id));
    let _ = fs::remove_dir_all(root);
}
...
fn permanent_delete_rejects_external_database_path() {
    ...
    let outside = root
        .parent()
        .expect("temporary parent")
        .join(format!("novelforge-external-{}", storage::new_id()));
    ...
    assert!(result.is_err());
    assert_eq!(
        fs::read_to_string(&sentinel).expect("external sentinel remains"),
        "keep"
    );
    let _ = fs::remove_dir_all(root);
    let _ = fs::remove_dir_all(outside);
