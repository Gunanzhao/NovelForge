...
describe('browser fallback project workflow', () => {
  ...
  it('keeps markdown titles and history aligned with desktop editing rules', async () => {
    ...
    await expect(fallbackInvoke<ProjectData>('restore_history', { input: { projectPath: path, revisionId: history[history.length - 1].id, expectedNodeId: 'another-chapter' } })).rejects.toThrow('不属于当前章节')
    await fallbackInvoke<ProjectData>('restore_history', { input: { projectPath: path, revisionId: history[history.length - 1].id, expectedNodeId: chapter.id } })
