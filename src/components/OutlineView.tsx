...
export function OutlineView() {
  ...
  const selectedTargetJson = JSON.stringify(targets.find((target) => target.key === selectedTargetKey) ?? null)
  const selectedTarget = useMemo(() => JSON.parse(selectedTargetJson) as OutlineTarget | undefined, [selectedTargetJson])
  ...
  async function save() {
    if (!selectedTarget || !draft.title.trim()) return false
  ...
  function openOutlineMenu(event: ReactMouseEvent<HTMLButtonElement>, target: OutlineTarget) {
    ...
    const items: ContextMenuItem[] = [
      { type: 'item', id: 'outline-open', label: '打开／编辑', icon: BookOpen, onSelect: () => runGuarded(() => setSelectedTargetKey(target.key)) },
