...
import { Clipboard, FileArchive, FileText, Image, Paperclip, Plus, Save, Trash2 } from 'lucide-react'
...
export function AttachmentsView() {
  ...
  useEffect(() => { setDescription(metadata(selected).description) }, [selected])
  ...
  async function saveDescription() {
    ...
    try {
      ...
      await saveEntity({
        ...
        content: { originalName: details.originalName, mimeType: details.mimeType, sizeBytes: Number(details.sizeBytes) || 0, description }, tags: ['附件'],
      })
    } catch (error) {
      setError(error)
    } finally {
  ...
  return <div className="workspace-view attachments-view"><div className="view-header"><div><p className="eyebrow">RESEARCH ATTACHMENTS</p><h1>资料附件</h1><p>把参考文档、图片和素材复制进项目目录，随项目一起备份；原始文件不会被修改。</p></div><div className="special-summary"><strong>{attachments.length}</strong><span>个附件</span></div></div><div className="attachments-toolbar"><TextInput value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="搜索附件名称或说明" /><Button onClick={() => void importFile()} disabled={busy}><Plus size={14} />{busy ? '导入中…' : '导入附件'}</Button></div><div className="attachments-layout"><aside className="attachments-list-pane"><div className="special-list">{visible.length ? visible.map((attachment) => { const details = metadata(attachment); const Icon = fileIcon(details.mimeType); return <button type="button" key={attachment.id} className={'attachment-list-item' + (attachment.id === selectedId ? ' active' : '')} onClick={() => setSelectedId(attachment.id)}><span className="attachment-icon"><Icon size={16} /></span><span><strong>{attachment.title}</strong><small>{bytes(details.sizeBytes)} · {details.mimeType}</small></span></button> }) : <div className="empty-state"><Paperclip size={25} /><div><strong>{filter ? '没有匹配附件' : '还没有附件'}</strong><span>{filter ? '换一个关键词试试。' : '导入研究资料、图片或参考文档。'}</span></div></div>}</div></aside><section className="special-editor"><Panel className="special-card">{selected ? <><div className="planning-card-head"><div><p className="eyebrow">ATTACHMENT DETAIL</p><h3>{selected.title}</h3></div><span className="planning-state">项目内副本</span></div><div className="planning-form"><div className="attachment-meta-grid"><div><span>文件类型</span><strong>{metadata(selected).mimeType}</strong></div><div><span>大小</span><strong>{bytes(metadata(selected).sizeBytes)}</strong></div></div><Field label="项目路径"><div className="input-with-action"><TextInput readOnly value={selected.filePath} /><Button variant="outline" onClick={copyPath}><Clipboard size={13} />复制</Button></div></Field><Field label="资料说明"><textarea className="text-area" value={description} onChange={(event) => setDescription(event.target.value)} placeholder="记录这份素材和小说的关系…" /></Field><div className="entity-actions"><Button onClick={() => void saveDescription()} disabled={busy}><Save size={14} />保存说明</Button><Button variant="danger" onClick={() => void remove()} disabled={busy}><Trash2 size={14} />移入回收站</Button></div></div></> : <div className="empty-state"><Paperclip size={25} /><div><strong>选择一个附件</strong><span>从左侧选择附件，或点击“导入附件”开始。</span></div></div>}</Panel></section></div></div>
}
