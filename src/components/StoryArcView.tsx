  ...
  return <div className="story-arc-view">
    ...
    <section className="story-arc-editor">
      ...
      <div className="story-arc-form" inert={busy}>
        ...
        <div className="entity-actions"><RecordSaveStatus updatedAt={selected?.updatedAt} busy={busy} /><Button disabled={busy || !title.trim()} onClick={() => void save()}><Save size={14} />{busy ? '保存中…' : '保存剧情线'}</Button>{selected ? <Button variant="danger" onClick={() => void remove()}><Trash2 size={14} />移入回收站</Button> : null}</div>
      </div>
    </section>
  </div>
}
