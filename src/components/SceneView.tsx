...
export function SceneView({ chapterId, onChapterChange }: { chapterId: string; onChapterChange: (id: string) => void }) {
  ...
  useEffect(() => {
    const next = creating ? blankDraft() : sceneDraft(selectedScene)
    setDraft(next)
  ...
  function openSceneMenu(event: MouseEvent, scene: EntityRecord) {
    ...
    const items: ContextMenuItem[] = [
      { type: 'item', id: 'scene-open', label: '打开／编辑', icon: BookOpen, onSelect: () => runGuarded(() => { setCreating(false); setEditingSceneId(scene.id) }) },
  ...
  function startNew() {
    runGuarded(() => { setCreating(true); setEditingSceneId(null) })
  }

  async function save() {
    if (!chapterId || !draft.title.trim()) return false
    ...
    try {
      ...
      return true
    } catch (error) { setError(error); return false } finally { setDraftSaving(draftId, false); setBusy(false) }
  }
  ...
  return <div className="scene-workspace"><div className="scene-list-column"><div className="planning-column-head"><div><p className="eyebrow">SCENE CARDS</p><h3>场景卡</h3></div><Button onClick={startNew}><Plus size={14} />新建场景</Button></div><select className="select-input" value={chapterId} onChange={(event) => { const value = event.target.value; runGuarded(() => { setCreating(false); setEditingSceneId(null); onChapterChange(value) }) }}><option value="">选择章节</option>{chapters.map((chapter) => <option key={chapter.id} value={chapter.id}>{chapter.title}</option>)}</select><div className="scene-list">{scenes.length ? scenes.map((scene, index) => <div key={scene.id} className={'scene-list-card' + (scene.id === editingSceneId && !creating ? ' active' : '')} draggable={!busy} onDragStart={() => setDraggedSceneId(scene.id)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); if (draggedSceneId) void persistOrder(reorderItems(scenes, draggedSceneId, scene.id)) }} onDragEnd={() => setDraggedSceneId(null)}><button className="scene-card-main" onClick={() => runGuarded(() => { setCreating(false); setEditingSceneId(scene.id) })}><GripVertical size={14} className="scene-drag-handle" /><span><strong>{index + 1}. {scene.title}</strong><small>{[contentText(scene, 'pov'), contentText(scene, 'location'), contentText(scene, 'goal')].filter(Boolean).join(' · ') || '尚未填写场景摘要'}</small></span></button><div className="scene-card-actions"><button aria-label="上移场景" title="上移场景" disabled={busy || index === 0} onClick={() => moveScene(scene.id, -1)}><ArrowUp size={13} /></button><button aria-label="下移场景" title="下移场景" disabled={busy || index === scenes.length - 1} onClick={() => moveScene(scene.id, 1)}><ArrowDown size={13} /></button></div></div>) : <div className="empty-state"><BookOpen size={24} /><div><strong>还没有场景卡</strong><span>场景卡可以拖动排序，并在保存后成为项目中的 Markdown 资料。</span></div></div>}</div></div><section className="scene-editor"><Panel className="planning-card">{chapterId ? <><div className="planning-card-head"><div><p className="eyebrow">SCENE DETAIL</p><h3>{creating ? '新建场景' : selectedScene?.title ?? '选择场景'}</h3></div><span className="planning-state">{busy ? '保存中…' : '本地资料'}</span></div><div className="planning-form" inert={busy}><Field label="场景标题"><TextInput autoFocus={creating} value={draft.title} onChange={(event) => updateField('title', event.target.value)} placeholder="例如：雾港的钟声" /></Field><div className="field-grid"><Field label="POV"><TextInput value={draft.pov} onChange={(event) => updateField('pov', event.target.value)} placeholder="视角人物" /></Field><Field label="地点"><TextInput value={draft.location} onChange={(event) => updateField('location', event.target.value)} placeholder="发生地点" /></Field></div><div className="field-grid"><Field label="时间"><TextInput value={draft.time} onChange={(event) => updateField('time', event.target.value)} placeholder="时间 / 时段" /></Field><Field label="参与人物"><TextInput value={draft.characters} onChange={(event) => updateField('characters', event.target.value)} placeholder="用逗号分隔人物" /></Field></div><Field label="目标"><textarea className="text-area" value={draft.goal} onChange={(event) => updateField('goal', event.target.value)} placeholder="场景中的角色想得到什么？" /></Field><Field label="冲突"><textarea className="text-area" value={draft.conflict} onChange={(event) => updateField('conflict', event.target.value)} placeholder="阻力、误会或对抗是什么？" /></Field><Field label="结果"><textarea className="text-area" value={draft.result} onChange={(event) => updateField('result', event.target.value)} placeholder="场景结束时发生了什么变化？" /></Field><Field label="备注"><textarea className="text-area compact" value={draft.notes} onChange={(event) => updateField('notes', event.target.value)} placeholder="写作提示…" /></Field><div className="entity-actions"><Button onClick={() => void save()} disabled={busy || !draft.title.trim()}><Save size={14} />{busy ? '保存中…' : '保存场景卡'}</Button>{selectedScene && !creating ? <Button variant="danger" onClick={() => void remove()}><Trash2 size={14} />移入回收站</Button> : null}</div></div></> : <div className="empty-state"><BookOpen size={25} /><div><strong>请先选择章节</strong><span>场景卡需要关联到一个章节。</span></div></div>}</Panel></section></div>
}
