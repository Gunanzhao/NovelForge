...
export function AiAssistantView() {
  ...
  useEffect(() => { writeAiPreferences({ endpoint, model }) }, [endpoint, model])
  ...
  async function runAssistant() {
    ...
    try {
      ...
      if (!endpoint.trim() || endpoint.trim().toLocaleLowerCase() === 'local' || !isDesktop) {
        ...
        setResult(await projectApi.aiComplete({ endpoint, apiKey, model, systemPrompt: '你是 NovelForge 的中文小说创作助手。只处理用户明确选中的上下文，不擅自引入未提供的事实。', prompt, temperature: 0.7, maxTokens: 4000 }))
      }
    } catch (error) {
      setError(error)
    } finally {
