...
export function Inspector() {
  ...
  const [nameKind, setNameKind] = useState<EntityKind>('character')
  ...
  async function createName(name: string) {
    try {
      await saveEntity({ projectPath: currentProjectPath, kind: nameKind, title: name, id: null, content: { summary: '由 NovelForge 本地规则生成，可继续编辑。' }, tags: ['待完善'] })
      selectEntity(nameKind)
    } catch (error) { setError(error) }
  }
