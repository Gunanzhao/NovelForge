  ...
  return <div className="entity-view">
    <div className="entity-layout">
      ...
      <section className="entity-editor">
        {selected || selectedEntityId === null ? <><div className="entity-editor-header"><div><p className="eyebrow">{ENTITY_LABELS[kind].toUpperCase()} ARCHIVE</p><h1>{selected ? selected.title : '新建' + ENTITY_LABELS[kind]}</h1><p>{selected ? '完善设定、查看关联，随时回到正文继续写作。' : '字段可以随时补充；先保存一个名字也可以。'}</p></div><div className="view-actions">{selected ? <Button variant="outline" onClick={() => void navigator.clipboard?.writeText('[[' + selected.title + ']]')}><Copy size={14} />复制 Wiki 链接</Button> : null}</div></div>
