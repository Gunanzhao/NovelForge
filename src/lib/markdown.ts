...
/**
 * Locate Wiki links outside code, metadata and Markdown destinations.
 *
...
export function wikiRanges(markdown: string): WikiRange[] {
  const protectedRanges = protectedMarkdownRanges(markdown, false)
  const ranges: WikiRange[] = []
  for (const match of markdown.matchAll(/\[\[([^[\]\r\n]+)\]\]/gu)) {
    const from = match.index
    const to = from + match[0].length
    const target = match[1].trim()
    if (!target || protectedRanges.some((range) => from < range.end && to > range.start)) continue
    ranges.push({ from, to, target })
  }
  return ranges
}
...
function protectWidthSensitiveSegments(markdown: string) {
  const merged = protectedMarkdownRanges(markdown)
