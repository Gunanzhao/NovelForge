...
async function run() {
  ...
  try {
    ...
    await sleep(500)
    await clickSelector(page, '.entity-list-head button', '新建')
    await setField(page, '输入人物名称', '苏岚')
    await clickSelector(page, '.entity-actions button', '保存资料')
    await waitForText(page, '苏岚')
    await sleep(500)
    ...
    await replaceEditor(page, '# 第一章\n\n林月来到雾港，随后查看[[林月]]档案。苏岚推开青石客栈，她又来到白石城。\n\n**关键线索**\n\n- 第一项\n- 第二项')
    await waitForCondition(page, "Array.from(document.querySelectorAll('.mention-row.known')).some((item)=>(item.textContent||'').includes('林月')) && Array.from(document.querySelectorAll('.mention-row.known')).some((item)=>(item.textContent||'').includes('苏岚')) && Array.from(document.querySelectorAll('.mention-row.known')).some((item)=>(item.textContent||'').includes('雾港'))", '已建档人物和地点自动识别')
    ...
    await waitForCondition(page, "Array.from(document.querySelectorAll('.mention-candidate')).some((item)=>(item.textContent||'').includes('白石城'))", '永久忽略候选识别')
    const ignoredMention = await page.evaluate("(()=>{const row=Array.from(document.querySelectorAll('.mention-candidate')).find((item)=>(item.textContent||'').includes('白石城'));const button=Array.from(row?.querySelectorAll('button')||[]).find((item)=>(item.textContent||'').trim()==='永久忽略');button?.click();return Boolean(button)})()")
    if (!ignoredMention) throw new Error('无法永久忽略自动提及候选')
    await waitForCondition(page, "Array.from(document.querySelectorAll('.mention-candidate')).every((item)=>!(item.textContent||'').includes('白石城'))", '永久忽略候选移除')
    ...
    await sleep(400)
    await clickSelectorContains(page, '.tree-main-button', '第二章')
    await waitForCondition(page, "document.querySelector('.editor-title-row h1')?.textContent?.trim() === '第二章'", 'Checklist 切换到第二章')
    await clickSelectorContains(page, '.tree-main-button', '第一章')
    await waitForCondition(page, "document.querySelector('.chapter-checklist-items input')?.checked === true", 'Checklist 切回第一章仍保留')
    ...
    await selectValue(page, 'select[aria-label="剧情节点状态"]', 'completed')
    ...
    await clickSelectorContains(page, '.story-arc-chapters button', '跳转')
    await waitForSelector(page, '.cm-content', '剧情线章节跳转')
    await clickExact(page, '剧情线')
    await clickSelectorContains(page, '.story-arc-list-item', '失落王冠主线')
    ...
    await clickText(page, '人物')
    await clickSelectorContains(page, '.entity-list-item', '林月')
    await waitForSelector(page, '.character-appearance-panel .appearance-metrics', '人物出场统计重开')
    ...
    await replaceEditor(page, '# 第一章\n\n林月来到雾港，白石城依旧只作为普通词语出现。\n\n**关键线索**')
    await sleep(700)
    await waitForCondition(page, "Array.from(document.querySelectorAll('.mention-candidate')).every((item)=>!(item.textContent||'').includes('白石城'))", '重启后永久忽略仍有效')
    await clickExact(page, '地点')
    await waitForCondition(page, "Array.from(document.querySelectorAll('.entity-list-item')).some((item)=>(item.textContent||'').includes('青石客栈'))", '重启后自动创建地点持久化')
    ...
    await clickSelectorContains(page, '.story-arc-list-item', '失落王冠主线')
    await waitForCondition(page, "document.querySelector('.story-arc-chapters input')?.checked === true && document.querySelectorAll('.story-arc-milestone').length === 2 && document.querySelector('select[aria-label=\"剧情节点状态\"]')?.value === 'completed'", '重启后剧情线章节、节点和完成状态持久化')
    ...
    await waitForCondition(page, "Array.from(document.querySelectorAll('.workflow-volume')).some((item)=>(item.textContent||'').includes('第一卷') && (item.textContent||'').includes('正文完成') && (item.textContent||'').includes('1 / 2'))", 'Dashboard Checklist 汇总数字')
