...
const server=createServer((request,response)=>{
  ...
  request.on('end',()=>{
    ...
    setTimeout(()=>{response.writeHead(200,{'Content-Type':'application/json'});response.end(JSON.stringify({choices:[{message:{content:'风很暖。灯很亮。'},finish_reason:'stop'}],model:'synthetic-writer'}))},4000)
  })
})
...
try {
  ...
  await click('运行辅助')
  for(let i=0;i<100&&requests.length===0;i++)await sleep(20)
  ...
  console.log('NATIVE_RESPONSIVE_MS',Date.now()-responsiveStart)
  await click('完整工作台')
