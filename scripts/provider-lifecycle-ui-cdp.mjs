...
const server=createServer((request,response)=>{
  ...
  request.on('end',()=>{
    ...
    response.end(JSON.stringify({choices:[{message:{content:index===1?'风很暖。':'风很暖。灯很亮。'},finish_reason:index===4?undefined:index===1?'length':'stop'}],model:'synthetic-writer'}))
  })
})
...
try {
  ...
  await click('替换选区')
  await waitFor(`editor().state.doc.toString()===${JSON.stringify(original.replace('风很冷。灯很暗。','风很暖。灯很亮。'))}`)
  await capture('cancel-and-retry-success')
  await click('取消')
  await ev(`editor().focus();editor().dispatch({selection:{anchor:${start},head:${start+8}}})`)
  await click('使用当前选区/光标')
  await click('运行辅助')
  await waitFor(`document.querySelector('.ai-output-heading').textContent.includes('结果不完整')`)
  assert.equal(requests.length,4)
  assert.equal(await ev(`!![...document.querySelectorAll('.ai-result-actions button')].find(e=>/替换|插入|追加/.test(e.textContent))`),false)
  await capture('missing-finish-protected')
