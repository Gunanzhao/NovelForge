...
try {
  ...
  await cmd('Emulation.setDeviceMetricsOverride',{width:2560,height:900,deviceScaleFactor:1,mobile:false})
  await ev(`document.querySelector('.inspector-region [role="tab"]:last-child').click()`)
  await sleep(350)
  const panelWidth = () => ev(`document.querySelector('.ai-host-inline').getBoundingClientRect().width`)
  const initialWidth = await panelWidth()
  const drag = async delta => {
    const point = await ev(`(()=>{const h=document.querySelector('.ai-host-inline .panel-resize-handle'),r=h.getBoundingClientRect();const x=r.left+r.width/2,y=r.top+120;if(document.elementFromPoint(x,y)!==h)throw Error('Resize handle is covered');return {x,y}})()`)
    await cmd('Input.dispatchMouseEvent',{type:'mousePressed',...point,button:'left',buttons:1,clickCount:1})
    await cmd('Input.dispatchMouseEvent',{type:'mouseMoved',x:point.x+delta,y:point.y,button:'left',buttons:1})
    await cmd('Input.dispatchMouseEvent',{type:'mouseReleased',x:point.x+delta,y:point.y,button:'left',buttons:0,clickCount:1})
    await sleep(350)
  }
  await drag(100)
  assert.equal(await panelWidth(),initialWidth-100,'AI sidebar shrinks when dragging right')
  await drag(-60)
  assert.equal(await panelWidth(),initialWidth-40,'AI sidebar expands when dragging left')
  await ev(`document.querySelector('.ai-host-inline .panel-resize-handle').focus()`)
  await cmd('Input.dispatchKeyEvent',{type:'keyDown',key:'ArrowLeft',code:'ArrowLeft',windowsVirtualKeyCode:37})
  await cmd('Input.dispatchKeyEvent',{type:'keyUp',key:'ArrowLeft',code:'ArrowLeft',windowsVirtualKeyCode:37})
  await sleep(350)
  const resizedWidth=await panelWidth()
  assert.equal(resizedWidth,initialWidth-30,'Keyboard resize remains available')
  await capture('ai-after-drag')
  await ev(`document.querySelector('.ai-host [role="tab"]:first-child').click()`)
  await sleep(350)
  assert.equal(await ev(`document.querySelector('.inspector-region').getBoundingClientRect().width`),resizedWidth)
  await ev(`document.querySelector('.inspector-region [role="tab"]:last-child').click()`)
  await sleep(350)
  assert.equal(await panelWidth(),resizedWidth)
  assert.equal(await ev(`JSON.parse(localStorage.getItem('novelforge:workspace-preferences:v1')).inspectorWidth`),resizedWidth)
  await ev('location.reload()')
  await sleep(800);await waitFor(`!!document.querySelector('.recent-project')`)
  await ev(`document.querySelector('.recent-project').click()`)
  await waitFor(`!!document.querySelector('.cm-content')`)
  await waitFor(`document.querySelector('.inspector-region').getBoundingClientRect().width===${resizedWidth}`)
  assert.equal(await ev(`document.querySelector('.inspector-region').getBoundingClientRect().width`),resizedWidth)
  writeFileSync(resolve(run,'result.json'),JSON.stringify({result:'INSPECTOR_WIDTH_PASS',records,initialWidth,resizedWidth},null,2))
