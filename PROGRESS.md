...
- rc.3 前端自动门禁、Rust 静态检查与常规测试已通过；Cargo audit 已通过（exit 0，17 条 allowed warnings，与 rc.2 相同）；Windows 构建已通过，CDP 已完整通过，WebDriver 第二轮完整通过，三种桌面 E2E 均已通过，对应源码基线 CI 已通过；rc.3 的发布附件见 GitHub Release。
...
# V1.1 P2 剧情线（2026-09-05）

- 新增 `story-arc` 统一资料类型、专用视图、左侧入口、章节 Inspector 和命令面板入口。
- 支持五种状态、颜色/优先级、多章节关联、milestone 增删改、完成、按钮排序、拖拽排序和章节跳转。
- 前端与 Rust 一致性检查均覆盖四种剧情线健康问题。
- 浏览器 fallback 覆盖创建、编辑、多个剧情线、章节关联、删除和回收站恢复；Rust 验证无效引用、长期未推进和数据库镜像重建。
- 阶段门禁：23 个前端测试文件 / 128 项测试通过；Rust 44 项常规测试通过、1 项大型基准按设计忽略；typecheck、lint、cargo check 均通过。
# V1.1 P3 人物出场统计（2026-09-05）

- 人物资料页新增出场统计，提供首次/最近登场、出现章节、正文提及、共同出现人物和主要地点。
- 新增人物出场统计工作区和章节人物矩阵；矩阵按 40 章 × 12 人物分页。
...
# V1.1 P5 灵感 Inbox（2026-09-05）

- 新增左侧灵感箱和 `Ctrl+Shift+I` 快速记录弹窗，支持可选标题、正文和标签。
- Inbox 提供未整理/已整理分类、搜索、标签过滤、时间排序、删除及回收站恢复入口。
- 支持转人物、地点、世界观、场景卡、伏笔、普通笔记及指定剧情线 milestone。
