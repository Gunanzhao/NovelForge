...
## 1.1.1-rc.1（2026-09-08，预发布）

本版将本地 1.1.0-rc.14 统一改号为 1.1.1-rc.1，并正式汇总此前 rc.12–rc.14 的改动。

- Codex CLI 根据协议、实际配置和本地无工具行为验证自动适配，兼容基线 0.149.1 / 0.153.4；支持阶段诊断、取消、重验与七天缓存。
- AI 辅助采用左侧任务与参考资料、右侧结果的双栏布局，操作固定在底部；连接设置、模板和请求预览按需打开。
- 同一窗口内切换页面或 AI 模式保持 Codex 连接与模型状态，检查中离开页面继续执行；取消和配置变化仍使旧结果失效。
- 生成前保留实际登录、CLI、配置及权限核验，不自动安装 CLI、不回退到 API 计费。
- Windows 下载、验证记录与限制见 [发布说明](docs/releases/v1.1.1-rc.1.md)。


## 1.1.0-rc.14（本地构建，未发布）

- Codex 连接检查及状态移至窗口级内存 Store，切换页面不再丢失模型列表、登录诊断与生成资格。
- 检查中离开页面继续执行，回来接续进度；相同配置共享检查，取消或配置变化后丢弃旧响应。
- 生成连接失败后禁用旧资格并提示重新检查；保留后端每次生成的实际配置、登录和模型校验。


## 1.1.0-rc.13（本地构建，未发布）

- AI 辅助采用左侧任务与参考资料、右侧结果的双栏布局，两侧独立滚动，运行和应用操作固定在底部。
- 模式、模型和连接状态集中到紧凑工具栏，连接配置及模板编辑改为按需弹窗；关闭重开保留当前状态和未保存草稿。
- 常用任务提供续写、润色、摘要入口，其他任务收进下拉框；请求预览集中展示最终内容，内置任务结果支持重新生成。
- 适配小窗口与深色模式，保留流式输出、生成取消、模板确认、正文及选区变化保护。
- 本地测试与构建边界见 [AI 工作台验收](docs/audits/ai-workbench-2026-09-08.md)。

## 1.1.0-rc.12（本地构建，未发布）

- Codex CLI 通过真实版本识别、稳定协议匹配、隔离配置及本地行为检查自动适配；验证基线 0.149.1 / 0.153.4。
- 连接界面独立显示登录和生成资格，支持检查进度、取消、重新验证、新旧额度结构和失效模型重选。
- 增加有指纹及期限约束的验证缓存、子进程诊断分类、检查取消竞态防护及 Windows CLI 兼容测试矩阵。
- 不自动安装 CLI，不修改全局配置，不默认执行真实订阅生成。
