...
- **开发依赖安全**：更新 ESLint 间接依赖 `js-yaml`，修复合并键解析可能造成 CPU 耗尽的高危漏洞，保留 CI 安全审计。
...
以下修复位于源码 main 分支，下载区的 rc.3 安装包不包含这些后续修改：

- 编辑器限制为一个可见选区，避免 Ctrl+D 创建不可见的编辑目标，保留原生跨行高亮和撤销重做。
- 历史列表、预览和恢复绑定当前项目及章节；切换后丢弃过期请求，后端拒绝恢复其他章节的版本，修正 Diff 换行。
- 模板预览绑定项目会话、章节、正文版本和选区；关闭、切换或修改后旧预览失效，确认前再次核对。
- 润色、改写、扩写、缩写只处理选区，禁用“使用整章”；需要处理全章时先在编辑器中全选正文。小节的最近章节上下文按所属章节计算，异常节点不再回退到全书末尾。
- Vitest 升级至 4.1.11，修复 GHSA-82fw-gwwq-j7x9；为 GTK 所需的 glib 0.18.5 回补官方 RUSTSEC-2024-0429 指针修复，补丁位于 `src-tauri/vendor/glib`，许可证、原包 SHA-256 和来源随源码保留。CI 增加 Linux 优化编译回归，不全局忽略安全公告。
- 桌面回归脚本从项目读取版本号，并适配当前 AI 工作台布局。

GTK 和 urlpattern 依赖链仍有 `proc-macro-error` 及五个 `unic-*` 包停止维护的上游警告。它们不能通过直接替换不兼容主版本消除，也不代表已发现应用可被利用；保留审计输出，升级框架时继续复核。
