...
当前版本已覆盖构建任务文档中的 MVP、写作规划、关系图、一致性、附件、结构管理、多格式导出、AI 辅助、数据恢复与大型正文树性能能力；1000 章 / 100 万字真实文件与 SQLite 性能验收已通过，Windows x64 release EXE 与 NSIS 安装包已构建并完成进程级冒烟。直连 CDP、官方 Tauri WebDriver、原生文件选择器和恢复重启流程均有回归脚本；剩余发布门禁是 CodeMirror FPS/滚动体感，以及在实际阅读器中打开六种导出文件并核对视觉结构。没有 API Key 时核心写作流程和本地 AI 草稿模式完全可用。
