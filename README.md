...
- 回收站、Markdown / TXT / DOCX / EPUB / PDF 导出、本地规则名字生成器。
...
- 详细统计：查看近 30 日写作趋势、活跃天数和章节字数排行。
- AI 辅助：支持 OpenAI-compatible Provider、显式上下文预览、续写/润色/改写/摘要，以及不依赖 API Key 的本地草稿模式。
...
当前版本已覆盖 MVP 及 V0.2 写作规划、关系图、一致性、附件、多格式导出和 AI 辅助基础能力；1000 章 / 100 万字真实文件与 SQLite 性能验收已通过，Windows x64 release EXE 与 NSIS 安装包已构建并完成进程级冒烟。真实桌面鼠标级 E2E 仍需在带 WebView2 的 Windows 桌面上人工完成。没有 API Key 时核心写作流程和本地 AI 草稿模式完全可用。
