...
- 原因：早期环境没有 tauri-driver 或 msedgedriver；现已使用与 WebView2 151 匹配的官方驱动完成可重复窗口流程，但自动化驱动不能替代用户实际桌面渲染和外部阅读器体验。
- 影响：P3-02 仍保留人工收尾门禁；不能把进程 Responding、命令层耗时或结构化导出断言标记为 FPS/阅读器视觉验收。
- 日期：2026-08-31
