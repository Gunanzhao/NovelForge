...
## ADR-009：全角/半角转换默认只改字符，不改结构

- 问题：批量转换若触碰 Markdown 标记、代码或 URL，会产生不可见的格式和链接破坏。
- 最终选择：默认仅转换 ASCII 字母/数字与其全角对应字符；代码块、行内代码和 URL 使用占位保护；普通空格只有显式 `convertSpace` 才转为 U+3000；中文标点转换保持独立命令。
- 影响：转换可重复、可逆，且不会改变 Markdown 结构；Inspector 提供独立“字符转全角/半角”操作。
- 日期：2026-09-03

## ADR-010：RC 远程证据不得由历史 run 代替

- 问题：当前候选未推送时，历史 GitHub Actions run 或旧 release E2E 不能证明当前 HEAD。
- 最终选择：本地门禁和当前 release CDP 结果可以记录为已完成；当前 HEAD CI 与远程 tag 保留待授权状态，官方 WebDriver 原生附件选择器的系统竞态单独记录。
